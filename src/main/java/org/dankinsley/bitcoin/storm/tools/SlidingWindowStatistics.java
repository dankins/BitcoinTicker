 ...
 * https://github.com/nathanmarz/storm-starter/blob/master/src/jvm/storm/starter/tools/SlidingWindowCounter.java
 ...
 */
public final class SlidingWindowStatistics<T> implements Serializable {

    private static final long serialVersionUID = -2645063988768785810L;

    private SlotBasedStatistics<T> objMetrics;
    private int headSlot;
    private int tailSlot;
    private int windowLengthInSlots;

    public SlidingWindowStatistics(int windowLengthInSlots) {
        if (windowLengthInSlots < 2) {
            throw new IllegalArgumentException("Window length in slots must be at least two (you requested "
                + windowLengthInSlots + ")");
        }
        this.windowLengthInSlots = windowLengthInSlots;
        this.objMetrics = new SlotBasedStatistics<T>(this.windowLengthInSlots);
    ...
    public void addValue(String metric, Double value) {
    	objMetrics.updateStatistics(metric, value, headSlot);
    }
     ...
     * @return
     */
    public Map<String, TickerStatistics> getStatsThenAdvanceWindow() {
        Map<String, TickerStatistics> metrics = objMetrics.getStatistics();
        objMetrics.wipeZeros();
        objMetrics.wipeSlot(tailSlot);
        advanceHead();
        return metrics;
    }
